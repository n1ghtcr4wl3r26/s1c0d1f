...
public class FisAnulControldifAction extends Action {
    /**
        ...
        if (usuario != null) {
                if (bean.getBoton().equals("consultar")) 
                {
                    dui = Util.DevuelveDUI(bean.getKey_year(),bean.getKey_cuo(),bean.getReg_serial());
                        ...
                        link = "volver";
                } else if (bean.getBoton().equals("consultar_anul")) 
                {
                    dui = Util.DevuelveDUI(bean.getKey_year(),bean.getKey_cuo(),bean.getReg_serial());
                        if (dui.getMensaje().substring(0, 5).equals("ERROR")) {
                            bean.setMensaje(Util.creamensaje("2", dui.getMensaje().substring(5), "6"));
                            ...
                            Util.DevuelveAnulRes(bean);
                            
                        }
                        link = "anulControlDiferido";
                }
            else if (bean.getBoton().equals("guardar_anulacion")) 
            {
                res = Util.GuardaAnulacionControlDif(bean, usuario);
                
                if (res.substring(0, 5).equals("ERROR")) {
                    bean.setMensaje(Util.creamensaje("2", res.substring(5), "6"));
                } else {
                    bean.setMensaje(Util.creamensaje("3",
                                                     "Se registr&oacute; exitosamente la anulaci&oacute;n del Control Diferido " +
                                                     bean.getNrofis(), "6"));   
                    bean.setBoton("");
                    bean.setDeclaracion("");    
                    bean.setEstado("-");
