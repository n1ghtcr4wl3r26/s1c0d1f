...
public class FisCierreContDifAction extends Action {
    /**
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        if (usuario != null) {
            ...
            else if (bean.getBoton().equals("guardar_cierre")) 
            {
                res = Util.GuardacierreControlDif(bean, usuario);
                
                if (res.substring(0, 5).equals("ERROR")) {
                    bean.setMensaje(Util.creamensaje("2", res.substring(5), "6"));
                } else {
                    bean.setMensaje(Util.creamensaje("3",
                                                     "Se registr&oacute; exitosamente el cierre del Control Diferido " +
                                                     bean.getNrofis(), "6"));   
                    bean.setBoton("");
                    bean.setDeclaracion("");    
                    bean.setEstado("-");
