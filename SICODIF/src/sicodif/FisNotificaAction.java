...
public class FisNotificaAction extends Action {
    /**
                ...
                if (bean.getBoton().equals("consultar")) 
                {
                    dui = Util.DevuelveDUI(bean.getKey_year(),bean.getKey_cuo(),bean.getReg_serial());
                        if (dui.getMensaje().substring(0, 5).equals("ERROR")) {
                            ...
                            bean.setFecha_registro_control(dui.getFecha_registro_control());
                            bean.setEstado(Util.DevuelveEstado(bean.getKey_year(),bean.getKey_cuo(),bean.getReg_serial()));
                            Util.DevuelveEnmNot(bean);
                        }
                        link = "volver";
                }
                else if (bean.getBoton().equals("registrar")) 
                {
                    res = Util.NotificaControlDiferido(bean, usuario);
...
*/
                ...
                {
                    res = Util.NotificaControlDiferidoEnm(bean, usuario);
...
*/
                ...
                {   
                    FisNotificaForm bean2 = new FisNotificaForm();
                    request.setAttribute("FisNotificaForm",bean2);
                    link = "volver";
                }
